...
#include <iostream>
	...
	{
		W = std::sqrt(Max(T(0), T(1) + aMatrix.m11 + aMatrix.m22 + aMatrix.m33)) * T(0.5);
		X = std::sqrt(Max(T(0), T(1) + aMatrix.m11 - aMatrix.m22 - aMatrix.m33)) * T(0.5);
		Y = std::sqrt(Max(T(0), T(1) - aMatrix.m11 + aMatrix.m22 - aMatrix.m33)) * T(0.5);
		Z = std::sqrt(Max(T(0), T(1) - aMatrix.m11 - aMatrix.m22 + aMatrix.m33)) * T(0.5);
	...
	{
		return GetEulerAnglesRadians() * Math::RadToDeg;
	}
	...
	inline Vector3<T> Quaternion<T>::GetEulerAnglesRadians() const
	{
		...
		if (abs(sinp) >= T(1))
		{
			yaw = copysign(Math::Pi * T(0.5), sinp); // use 90 degrees if out of range //pitch
		}
		else
		{
			yaw = asin(sinp); // pitch
